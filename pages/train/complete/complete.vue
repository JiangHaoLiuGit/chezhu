				...
				<mescroll-body ref="mescrollRef" @init="mescrollInit" :up="upOption"  @down="downCallback" @up="upCallback">
					<view class="item u-flex" v-for="(item,index) in order" @click="to(item)">
						<view class="itemLeft">
							...
							<view class="small yellow" v-if="item.completeRate == 0">NEW</view>
							<view class="small blue" v-else-if="item.completeRate < 100">未完成</view>
							<view v-else-if="item.completeRate == 100">
								<view class="small yellow" v-if="item.status == 2">考试待完成</view>
								<view class="small blue" v-else-if="item.status == 3">审核中</view>
								<view class="small yellow" v-else-if="item.status == 4">完成</view>
								<view class="small blue" v-else-if="item.status == 5">审核未通过</view>
							</view>
						...
						<view class="itemRight">
							...
							<view class="u-flex u-row-between">
								<view>
									...
									<view class="tooSmallText nineColor" v-if="item.status == 4">
										通过日期：{{item.planEndDate}}
									</view>
									<view class="tooSmallText nineColor" v-else>
										完成日期：{{item.planEndDate}}
									</view>
								</view>
								<view v-if="item.status != 4">
									<view class="btn" v-if="item.completeRate < 100">
										开始培训
									</view>
